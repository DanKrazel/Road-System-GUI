...
public class Vehicle implements Utilities, Timer {
	...
	public synchronized void updatelocation() {
        if (currentRoutePart instanceof Road) {
            double tempPlace = timeOnCurrentPart * Math.min(getLastRoad().getMaxSpeed(), vehicleType.getAverageSpeed());
            double tempLen = lastRoad.getLength() - tempPlace;
            X = ((lastRoad.getStartJunction().getX() * tempLen + lastRoad.getEndJunction().getX() * tempPlace) / (tempPlace + tempLen));
            Y = ((lastRoad.getStartJunction().getY() * tempLen + lastRoad.getEndJunction().getY() * tempPlace) / (tempPlace + tempLen));
        }
